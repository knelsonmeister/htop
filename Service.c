...
static uid_t Service_getuid = (uid_t)-1;

int Service_pidDigits = 7;

void Service_setupColumnWidths() {
   int maxPid = Platform_getMaxPid();
   if (maxPid == -1)
      return;

   Service_pidDigits = ceil(log10(maxPid));
   assert(Service_pidDigits <= PROCESS_MAX_PID_DIGITS);
}
...
void Service_printKBytes(RichString* str, unsigned long long number, bool coloring) {
   if (number == ULLONG_MAX)
      Service_printBytes(str, ULLONG_MAX, coloring);
   else
      Service_printBytes(str, number * ONE_K, coloring);
}
 ...
 * Service_writeCommand() for coloring. The merged Command string is also
 * returned by Service_getCommandStr() for searching, sorting and filtering.
 */
void Service_makeCommandStr(Service* this) {
   ServiceMergedCommand* mc = &this->mergedCommand;
   ...
   if (Service_isKernelThread(this))
      return;
   if (this->state == 'Z' && !this->mergedCommand.str)
      return;
   if (Service_isUserlandThread(this) && settings->showThreadNames && (showThreadNames == mc->prevShowThreadNames))
      return;
   ...
   const int baseAttr = Service_isThread(this) ? CRT_colors[PROCESS_THREAD_BASENAME] : CRT_colors[PROCESS_BASENAME];
   const int commAttr = Service_isThread(this) ? CRT_colors[PROCESS_THREAD_COMM] : CRT_colors[PROCESS_COMM];
   ...
   if (!showMergedCommand || !procExe || !procComm) { /* fall back to cmdline */
      if (showMergedCommand && (!Service_isUserlandThread(this) || showThreadNames) && !procExe && procComm && strlen(procComm)) { /* Prefix column with comm */
   ...
   if (procExe && procComm && (!Service_isUserlandThread(this) || showThreadNames)) {
   ...
   if (searchCommInCmdline) {
      ...
      haveCommInCmdline = (!Service_isUserlandThread(this) || showThreadNames) && findCommInCmdline(procComm, cmdline, cmdlineBasenameStart, &commStart, &commEnd);
   }
   ...
   if (!haveCommInExe && !haveCommInCmdline && procComm && (!Service_isUserlandThread(this) || showThreadNames)) {
      WRITE_SEPARATOR;
      WRITE_HIGHLIGHT(0, strlen(procComm), commAttr, CMDLINE_HIGHLIGHT_FLAG_COMM);
      str = stpcpy(str, procComm);
      haveCommField = true;
   ...
   if (!haveCommInExe && haveCommInCmdline && !haveCommField && (!Service_isUserlandThread(this) || showThreadNames))
...
void Service_writeCommand(const Service* this, int attr, int baseAttr, RichString* str) {
   ...
   const ServiceMergedCommand* mc = &this->mergedCommand;
...
void Service_writeField(const Service* this, RichString* str, ServiceField field) {
   ...
   case COMM: {
      ...
      if (this->settings->highlightThreads && Service_isThread(this)) {
         attr = CRT_colors[PROCESS_THREAD];
         baseattr = CRT_colors[PROCESS_THREAD_BASENAME];
      }
      if (!this->settings->treeView || this->indent == 0) {
         Service_writeCommand(this, attr, baseattr, str);
         return;
      }
   ...
   case PROC_COMM: {
      const char* procComm;
      if (this->procComm) {
         attr = CRT_colors[Service_isUserlandThread(this) ? PROCESS_THREAD_COMM : PROCESS_COMM];
         procComm = this->procComm;
      } else {
         attr = CRT_colors[PROCESS_SHADOW];
         procComm = Service_isKernelThread(this) ? kthreadID : "N/A";
      }

      Service_printLeftAlignedField(str, attr, procComm, TASK_COMM_LEN - 1);
      return;
   }
   case PROC_EXE: {
      const char* procExe;
      if (this->procExe) {
         attr = CRT_colors[Service_isUserlandThread(this) ? PROCESS_THREAD_BASENAME : PROCESS_BASENAME];
         ...
         attr = CRT_colors[PROCESS_SHADOW];
         procExe = Service_isKernelThread(this) ? kthreadID : "N/A";
      }

      Service_printLeftAlignedField(str, attr, procExe, TASK_COMM_LEN - 1);
      return;
   }
   case CWD: {
      const char* cwd;
      ...
      Service_printLeftAlignedField(str, attr, cwd, 25);
      return;
   }
   case ELAPSED: Service_printTime(str, /* convert to hundreds of a second */ this->serviceList->realtimeMs / 10 - 100 * this->starttime_ctime, coloring); return;
   case MAJFLT: Service_printCount(str, this->majflt, coloring); return;
   case MINFLT: Service_printCount(str, this->minflt, coloring); return;
   case M_RESIDENT: Service_printKBytes(str, this->m_resident, coloring); return;
   case M_VIRT: Service_printKBytes(str, this->m_virt, coloring); return;
   case NICE:
   ...
   case NLWP:
      ...
      break;
   case PERCENT_CPU: Service_printPercentage(this->percent_cpu, buffer, n, &attr); break;
   case PERCENT_NORM_CPU: {
      float cpuPercentage = this->percent_cpu / this->serviceList->activeCPUs;
      Service_printPercentage(cpuPercentage, buffer, n, &attr);
      break;
   }
   case PERCENT_MEM: Service_printPercentage(this->percent_mem, buffer, n, &attr); break;
   case PGRP: xSnprintf(buffer, n, "%*d ", Service_pidDigits, this->pgrp); break;
   case PID: xSnprintf(buffer, n, "%*d ", Service_pidDigits, this->pid); break;
   case PPID: xSnprintf(buffer, n, "%*d ", Service_pidDigits, this->ppid); break;
   case PRIORITY:
   ...
   case SESSION: xSnprintf(buffer, n, "%*d ", Service_pidDigits, this->session); break;
   ...
   case TIME: Service_printTime(str, this->time, coloring); return;
   case TGID:
      if (this->tgid == this->pid)
         attr = CRT_colors[PROCESS_SHADOW];

      xSnprintf(buffer, n, "%*d ", Service_pidDigits, this->tgid);
      break;
   case TPGID: xSnprintf(buffer, n, "%*d ", Service_pidDigits, this->tpgid); break;
   case TTY:
      if (!this->tty_name) {
         attr = CRT_colors[PROCESS_SHADOW];
      ...
      break;
   case USER:
      if (Service_getuid != this->st_uid)
         attr = CRT_colors[PROCESS_SHADOW];

      if (this->user) {
         Service_printLeftAlignedField(str, attr, this->user, 9);
         return;
      }
   ...
   default:
      ...
      //    return;
      assert(0 && "Service_writeField: default key reached"); /* should never be reached */
      xSnprintf(buffer, n, "- ");
      break;
   }
...
void Service_display(const Object* cast, RichString* out) {
   const Service* this = (const Service*) cast;
   const ServiceField* fields = this->settings->fields;
   for (int i = 0; fields[i]; i++)
      As_Service(this)->writeField(this, out, fields[i]);

   if (this->settings->shadowOtherUsers && this->st_uid != Service_getuid) {
   ...
   if (this->settings->highlightChanges) {
      if (Service_isTomb(this)) {
         out->highlightAttr = CRT_colors[PROCESS_TOMB];
      } else if (Service_isNew(this)) {
 ...
 * this follows Service_writeField(COMM) and Service_writeCommand */
const char* Service_getCommandStr(const Service* this) {
   if ((Service_isUserlandThread(this) && this->settings->showThreadNames) || !this->mergedCommand.str) {
      return this->cmdline;
   }
...
const ServiceClass Service_class = {
   ...
   .writeField = Service_writeField,
   .getCommandStr = Service_getCommandStr,
};

void Service_init(Service* this, const Settings* settings) {
   ...
   if (Service_getuid == (uid_t)-1) {
      Service_getuid = getuid();
   }
}
...
bool Service_isNew(const Service* this) {
   assert(this->serviceList);
   if (this->serviceList->monotonicMs >= this->seenStampMs) {
      return this->serviceList->monotonicMs - this->seenStampMs <= 1000 * (uint64_t)this->serviceList->settings->highlightDelaySecs;
   }
   return false;
}
...
bool Service_changePriorityBy(Service* this, Arg delta) {
   return Service_setPriority(this, this->nice + delta.i);
}
...
int Service_compare(const void* v1, const void* v2) {
   ...
   int result = Service_compareByKey(p1, p2, key);
...
int Service_compareByKey_Base(const Service* p1, const Service* p2, ServiceField key) {
   ...
   case COMM:
      return SPACESHIP_NULLSTR(Service_getCommand(p1), Service_getCommand(p2));
   case PROC_COMM: {
      const char* comm1 = p1->procComm ? p1->procComm : (Service_isKernelThread(p1) ? kthreadID : "");
      const char* comm2 = p2->procComm ? p2->procComm : (Service_isKernelThread(p2) ? kthreadID : "");
      return SPACESHIP_NULLSTR(comm1, comm2);
   }
   case PROC_EXE: {
      const char* exe1 = p1->procExe ? (p1->procExe + p1->procExeBasenameOffset) : (Service_isKernelThread(p1) ? kthreadID : "");
      const char* exe2 = p2->procExe ? (p2->procExe + p2->procExeBasenameOffset) : (Service_isKernelThread(p2) ? kthreadID : "");
   ...
   default:
      assert(0 && "Service_compareByKey_Base: default key reached"); /* should never be reached */
      return SPACESHIP_NUMBER(p1->pid, p2->pid);
   }
}
