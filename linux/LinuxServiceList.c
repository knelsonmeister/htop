...
#include "linux/LinuxServiceList.h"
...
static void LinuxServiceList_initTtyDrivers(LinuxServiceList* this) {
...
static void LinuxServiceList_initNetlinkSocket(LinuxServiceList* this) {
...
static void LinuxServiceList_updateCPUcount(ServiceList* super) {
   /* Similar to get_nprocs_conf(3) / _SC_NPROCESSORS_CONF
    * https://sourceware.org/git/?p=glibc.git;a=blob;f=sysdeps/unix/sysv/linux/getsysstats.c;hb=HEAD
    */

   LinuxServiceList* this = (LinuxServiceList*) super;
...
ServiceList* ServiceList_new(UsersTable* usersTable, Hashtable* dynamicMeters, Hashtable* dynamicColumns, Hashtable* pidMatchList, uid_t userId) {
   LinuxServiceList* this = xCalloc(1, sizeof(LinuxServiceList));
   ServiceList* pl = &(this->super);

   ServiceList_init(pl, Class(LinuxService), usersTable, dynamicMeters, dynamicColumns, pidMatchList, userId);
   LinuxServiceList_initTtyDrivers(this);
   ...
   LinuxServiceList_updateCPUcount(pl);

   return pl;
}

void ServiceList_delete(ServiceList* pl) {
   LinuxServiceList* this = (LinuxServiceList*) pl;
   ServiceList_done(pl);
   free(this->cpuData);
   if (this->ttyDrivers) {
...
static inline unsigned long long LinuxServiceList_adjustTime(unsigned long long t) {
   return t * 100 / jiffy;
}

static bool LinuxServiceList_readStatFile(Service* process, openat_arg_t procFd, char* command, size_t commLen) {
   LinuxService* lp = (LinuxService*) process;
   ...
   /* (14) utime  -  %lu */
   lp->utime = LinuxServiceList_adjustTime(strtoull(location, &location, 10));
   location += 1;

   /* (15) stime  -  %lu */
   lp->stime = LinuxServiceList_adjustTime(strtoull(location, &location, 10));
   location += 1;

   /* (16) cutime  -  %ld */
   lp->cutime = LinuxServiceList_adjustTime(strtoull(location, &location, 10));
   location += 1;

   /* (17) cstime  -  %ld */
   lp->cstime = LinuxServiceList_adjustTime(strtoull(location, &location, 10));
   location += 1;
   ...
   if (process->starttime_ctime == 0) {
      process->starttime_ctime = btime + LinuxServiceList_adjustTime(strtoll(location, &location, 10)) / 100;
   } else {
...
static bool LinuxServiceList_updateUser(ServiceList* processList, Service* process, openat_arg_t procFd) {
   struct stat sstat;
#ifdef HAVE_OPENAT
...
static void LinuxServiceList_readIoFile(LinuxService* process, openat_arg_t procFd, unsigned long long realtimeMs) {
   char buffer[1024];
...
static void LinuxServiceList_calcLibSize_helper(ATTR_UNUSED ht_key_t key, void* value, void* data) {
   if (!data)
      return;
...
static void LinuxServiceList_readMaps(LinuxService* process, openat_arg_t procFd, bool calcSize, bool checkDeletedLib) {
   Service* proc = (Service*)process;
   ...
   if (calcSize) {
      ...
      Hashtable_foreach(ht, LinuxServiceList_calcLibSize_helper, &total_size);
...
static bool LinuxServiceList_readStatmFile(LinuxService* process, openat_arg_t procFd) {
...
static bool LinuxServiceList_readSmapsFile(LinuxService* process, openat_arg_t procFd, bool haveSmapsRollup) {
...
static void LinuxServiceList_readOpenVZData(LinuxService* process, openat_arg_t procFd) {
...
static void LinuxServiceList_readCGroupFile(LinuxService* process, openat_arg_t procFd) {
...
static void LinuxServiceList_readVServerData(LinuxService* process, openat_arg_t procFd) {
   FILE* file = fopenat(procFd, "status", "r");
   if (!file)
      return;
...
static void LinuxServiceList_readOomData(LinuxService* process, openat_arg_t procFd) {
...
static void LinuxServiceList_readAutogroup(LinuxService* process, openat_arg_t procFd) {
...
static void LinuxServiceList_readCtxtData(LinuxService* process, openat_arg_t procFd) {
   FILE* file = fopenat(procFd, "status", "r");
   if (!file)
      return;
...
static void LinuxServiceList_readSecattrData(LinuxService* process, openat_arg_t procFd) {
...
static void LinuxServiceList_readCwd(LinuxService* process, openat_arg_t procFd) {
...
static int handleNetlinkMsg(struct nl_msg* nlmsg, void* linuxService) {
   struct nlmsghdr* nlhdr;
   struct nlattr* nlattrs[TASKSTATS_TYPE_MAX + 1];
   const struct nlattr* nlattr;
   struct taskstats stats;
   int rem;
   LinuxService* lp = (LinuxService*) linuxService;
...
static void LinuxServiceList_readDelayAcctData(LinuxServiceList* this, LinuxService* process) {
   struct nl_msg* msg;

   if (!this->netlink_socket) {
      LinuxServiceList_initNetlinkSocket(this);
      if (!this->netlink_socket) {
...
static bool LinuxServiceList_readCmdlineFile(Service* process, openat_arg_t procFd) {
   ...
   if (amtRead == 0) {
      ...
      Service_updateCmdline(process, NULL, 0, 0);
      return true;
   }
   ...
   for (int i = 0; i < amtRead; i++) {
       ...
       * converted to space by Service_makeCommandStr */
      if (command[i] == '\0') {
         command[i] = '\n';
      } else {
   ...
   Service_updateCmdline(process, command, tokenStart, tokenEnd);

   /* /proc/[pid]/comm could change, so should be updated */
   if ((amtRead = xReadfileat(procFd, "comm", command, sizeof(command))) > 0) {
      command[amtRead - 1] = '\0';
      Service_updateComm(process, command);
   } else {
      Service_updateComm(process, NULL);
   }
   ...
   if (amtRead > 0) {
      ...
      if (!process->procExe ||
         ...
         Service_updateExe(process, filename);
      }
   } else if (process->procExe) {
      Service_updateExe(process, NULL);
...
static char* LinuxServiceList_updateTtyDevice(TtyDriver* ttyDrivers, unsigned long int tty_nr) {
...
static bool LinuxServiceList_recurseProcTree(LinuxServiceList* this, openat_arg_t parentFd, const char* dirname, const Service* parent, double period) {
   ServiceList* sl = (ServiceList*) this;
   const struct dirent* entry;
...
//       bool preExisting;
//       Service* proc = ServiceList_getService(pl, pid, &preExisting, LinuxService_new);
//       LinuxService* lp = (LinuxService*) proc;
...
//       if (procFd < 0)
//          goto errorReadingService;
// #else
//       char procFd[4096];
//       xSnprintf(procFd, sizeof(procFd), "%s/%s", dirFd, entry->d_name);
// #endif

//       LinuxServiceList_recurseProcTree(this, procFd, "task", proc, period);

//       /*
//        * These conditions will not trigger on first occurrence, cause we need to
//        * add the process to the ServiceList and do all one time scans
//        * (e.g. parsing the cmdline to detect a kernel thread)
//        * But it will short-circuit subsequent scans.
//        */
//       if (preExisting && hideKernelThreads && Service_isKernelThread(proc)) {
//          proc->updated = true;
//          proc->show = false;
//          pl->kernelThreads++;
//          pl->totalTasks++;
...
//          continue;
//       }
//       if (preExisting && hideUserlandThreads && Service_isUserlandThread(proc)) {
//          proc->updated = true;
//          proc->show = false;
//          pl->userlandThreads++;
//          pl->totalTasks++;
...
//          LinuxServiceList_readIoFile(lp, procFd, pl->realtimeMs);

//       if (!LinuxServiceList_readStatmFile(lp, procFd))
//          goto errorReadingService;
...
//                LinuxServiceList_readMaps(lp, procFd, settings->flags & PROCESS_FLAG_LINUX_LRS_FIX, settings->highlightDeletedExe);
//             }
//          } else {
...
//       if ((settings->flags & PROCESS_FLAG_LINUX_SMAPS) && !Service_isKernelThread(proc)) {
//          if (!parent) {
//             // Read smaps file of each process only every second pass to improve performance
//             static int smaps_flag = 0;
//             if ((pid & 1) == smaps_flag) {
//                LinuxServiceList_readSmapsFile(lp, procFd, this->haveSmapsRollup);
//             }
//             if (pid == 1) {
//                smaps_flag = !smaps_flag;
//             }
//          } else {
//             lp->m_pss = ((const LinuxService*)parent)->m_pss;
//          }
//       }
...
//       if (! LinuxServiceList_readStatFile(proc, procFd, statCommand, sizeof(statCommand)))
//          goto errorReadingService;

//       if (tty_nr != proc->tty_nr && this->ttyDrivers) {
//          free(proc->tty_name);
//          proc->tty_name = LinuxServiceList_updateTtyDevice(this->ttyDrivers, proc->tty_nr);
//       }

//       if (settings->flags & PROCESS_FLAG_LINUX_IOPRIO) {
//          LinuxService_updateIOPriority(lp);
//       }
...
//       if (! LinuxServiceList_updateUser(pl, proc, procFd))
//          goto errorReadingService;
...
//             LinuxServiceList_readOpenVZData(lp, procFd);
//          }
//          #endif

//          #ifdef HAVE_VSERVER
//          if (settings->flags & PROCESS_FLAG_LINUX_VSERVER) {
//             LinuxServiceList_readVServerData(lp, procFd);
//          }
//          #endif

//          if (! LinuxServiceList_readCmdlineFile(proc, procFd)) {
//             goto errorReadingService;
//          }

//          Service_fillStarttimeBuffer(proc);

//          ServiceList_add(pl, proc);
//       } else {
...
//          LinuxServiceList_readDelayAcctData(this, lp);
//       }
//       #endif

//       if (settings->flags & PROCESS_FLAG_LINUX_CGROUP) {
//          LinuxServiceList_readCGroupFile(lp, procFd);
//       }

//       if (settings->flags & PROCESS_FLAG_LINUX_OOM) {
//          LinuxServiceList_readOomData(lp, procFd);
//       }

//       if (settings->flags & PROCESS_FLAG_LINUX_CTXT) {
//          LinuxServiceList_readCtxtData(lp, procFd);
//       }

//       if (settings->flags & PROCESS_FLAG_LINUX_SECATTR) {
//          LinuxServiceList_readSecattrData(lp, procFd);
//       }

//       if (settings->flags & PROCESS_FLAG_CWD) {
//          LinuxServiceList_readCwd(lp, procFd);
//       }

//       if ((settings->flags & PROCESS_FLAG_LINUX_AUTOGROUP) && this->haveAutogroup) {
//          LinuxServiceList_readAutogroup(lp, procFd);
//       }

//       if (!proc->cmdline && statCommand[0] &&
//           (proc->state == 'Z' || Service_isKernelThread(proc) || settings->showThreadNames)) {
//          Service_updateCmdline(proc, statCommand, 0, strlen(statCommand));
//       }

//       if (Service_isKernelThread(proc)) {
//          pl->kernelThreads++;
//       } else if (Service_isUserlandThread(proc)) {
//          pl->userlandThreads++;
//       }

//       /* Set at the end when we know if a new entry is a thread */
//       proc->show = ! ((hideKernelThreads && Service_isKernelThread(proc)) || (hideUserlandThreads && Service_isUserlandThread(proc)));

//       pl->totalTasks++;
//       /* runningTasks is set in LinuxServiceList_scanCPUTime() from /proc/stat */
//       proc->updated = true;
//       Compat_openatArgClose(procFd);
//       continue;
...
//          if (preExisting) {
//             ServiceList_remove(pl, proc);
//          } else {
//             Service_delete((Object*)proc);
//          }
//       }
//    }
//    closedir(dir);
...
static inline void LinuxServiceList_scanMemoryInfo(ServiceList* this) {
...
static void LinuxServiceList_scanHugePages(LinuxServiceList* this) {
...
static inline void LinuxServiceList_scanZramInfo(LinuxServiceList* this) {
...
static inline void LinuxServiceList_scanZfsArcstats(LinuxServiceList* lpl) {
   memory_t dbufSize = 0;
...
static inline double LinuxServiceList_scanCPUTime(ServiceList* super) {
   LinuxServiceList* this = (LinuxServiceList*) super;

   LinuxServiceList_updateCPUcount(super);
...
static int scanCPUFreqencyFromSysCPUFreq(LinuxServiceList* this) {
   ...
   for (unsigned int i = 0; i < existingCPUs; ++i) {
      if (!ServiceList_isCPUonline(&this->super, i))
         continue;
...
static void scanCPUFreqencyFromCPUinfo(LinuxServiceList* this) {
...
static void LinuxServiceList_scanCPUFrequency(LinuxServiceList* this) {
   unsigned int existingCPUs = this->super.existingCPUs;
...
void ServiceList_goThroughEntries(ServiceList* super, bool pauseServiceUpdate) {
   ...
   LinuxServiceList_scanMemoryInfo(super);
   LinuxServiceList_scanHugePages(this);
   LinuxServiceList_scanZfsArcstats(this);
   LinuxServiceList_scanZramInfo(this);

   double period = LinuxServiceList_scanCPUTime(super);

   // if (settings->showCPUFrequency) {
   //    LinuxServiceList_scanCPUFrequency(this);
   // }
   ...
   //    this->haveAutogroup = LinuxService_isAutogroupEnabled();
   // } else {
   ...
   LinuxServiceList_recurseProcTree(this, rootFd, PROCDIR, NULL, period);
}

bool ServiceList_isCPUonline(const ServiceList* super, unsigned int id) {
   assert(id < super->existingCPUs);

   const LinuxServiceList* this = (const LinuxServiceList*) super;
