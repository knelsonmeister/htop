...
const ServiceFieldData Service_fields[LAST_SERVICEFIELD] = {
   [0] = { .name = "", .title = NULL, .description = NULL, .flags = 0, },
   [PID] = { .name = "PID", .title = "PID", .description = "Service/thread ID", .flags = 0, .pidColumn = true, },
   [COMM] = { .name = "Command", .title = "Command ", .description = "Command line", .flags = 0, },
   [STATE] = { .name = "STATE", .title = "S ", .description = "Service state (S sleeping, R running, D disk, Z zombie, T traced, W paging, I idle)", .flags = 0, },
   [PPID] = { .name = "PPID", .title = "PPID", .description = "Parent process ID", .flags = 0, .pidColumn = true, },
   [PGRP] = { .name = "PGRP", .title = "PGRP", .description = "Service group ID", .flags = 0, .pidColumn = true, },
   [SESSION] = { .name = "SESSION", .title = "SID", .description = "Service's session ID", .flags = 0, .pidColumn = true, },
   [TTY] = { .name = "TTY", .title = "TTY      ", .description = "Controlling terminal", .flags = 0, },
   [TPGID] = { .name = "TPGID", .title = "TPGID", .description = "Service ID of the fg process group of the controlling terminal", .flags = 0, .pidColumn = true, },
...
Service* LinuxService_new(const Settings* settings) {
   LinuxService* this = xCalloc(1, sizeof(LinuxService));
   Object_setClass(this, Class(LinuxService));
   Service_init(&this->super, settings);
   return &this->super;
}

void Service_delete(Object* cast) {
   LinuxService* this = (LinuxService*) cast;
   Service_done((Service*)cast);
   free(this->cgroup);
#ifdef HAVE_OPENVZ
   free(this->ctid);
#endif
...
*/
static int LinuxService_effectiveIOPriority(const LinuxService* this) {
...
IOPriority LinuxService_updateIOPriority(LinuxService* this) {
   IOPriority ioprio = 0;
...
bool LinuxService_setIOPriority(Service* this, Arg ioprio) {
// Other OSes masquerading as Linux (NetBSD?) don't have this syscall
#ifdef SYS_ioprio_set
   syscall(SYS_ioprio_set, IOPRIO_WHO_PROCESS, this->pid, ioprio.i);
#endif
   return (LinuxService_updateIOPriority((LinuxService*)this) == ioprio.i);
}

bool LinuxService_isAutogroupEnabled(void) {
   char buf[16];
...
bool LinuxService_changeAutogroupPriorityBy(Service* this, Arg delta) {
   char buffer[256];
...
static void LinuxService_writeField(const Service* this, RichString* str, ServiceField field) {
   const LinuxService* lp = (const LinuxService*) this;
   bool coloring = this->settings->highlightMegabytes;
   char buffer[256]; buffer[255] = '\0';
   int attr = CRT_colors[DEFAULT_COLOR];
   size_t n = sizeof(buffer) - 1;
   switch (field) {
   case CMINFLT: Service_printCount(str, lp->cminflt, coloring); return;
   case CMAJFLT: Service_printCount(str, lp->cmajflt, coloring); return;
   case M_DRS: Service_printBytes(str, lp->m_drs * pageSize, coloring); return;
   case M_LRS:
      if (lp->m_lrs) {
         Service_printBytes(str, lp->m_lrs * pageSize, coloring);
         return;
      }
      ...
      break;
   case M_TRS: Service_printBytes(str, lp->m_trs * pageSize, coloring); return;
   case M_SHARE: Service_printBytes(str, lp->m_share * pageSize, coloring); return;
   case M_PSS: Service_printKBytes(str, lp->m_pss, coloring); return;
   case M_SWAP: Service_printKBytes(str, lp->m_swap, coloring); return;
   case M_PSSWP: Service_printKBytes(str, lp->m_psswp, coloring); return;
   case UTIME: Service_printTime(str, lp->utime, coloring); return;
   case STIME: Service_printTime(str, lp->stime, coloring); return;
   case CUTIME: Service_printTime(str, lp->cutime, coloring); return;
   case CSTIME: Service_printTime(str, lp->cstime, coloring); return;
   case RCHAR:  Service_printBytes(str, lp->io_rchar, coloring); return;
   case WCHAR:  Service_printBytes(str, lp->io_wchar, coloring); return;
   case SYSCR:  Service_printCount(str, lp->io_syscr, coloring); return;
   case SYSCW:  Service_printCount(str, lp->io_syscw, coloring); return;
   case RBYTES: Service_printBytes(str, lp->io_read_bytes, coloring); return;
   case WBYTES: Service_printBytes(str, lp->io_write_bytes, coloring); return;
   case CNCLWB: Service_printBytes(str, lp->io_cancelled_write_bytes, coloring); return;
   case IO_READ_RATE:  Service_printRate(str, lp->io_rate_read_bps, coloring); return;
   case IO_WRITE_RATE: Service_printRate(str, lp->io_rate_write_bps, coloring); return;
   case IO_RATE: {
      double totalRate;
      if (!isnan(lp->io_rate_read_bps) && !isnan(lp->io_rate_write_bps))
         ...
         totalRate = NAN;
      Service_printRate(str, totalRate, coloring);
      return;
   }
   #ifdef HAVE_OPENVZ
   case CTID: xSnprintf(buffer, n, "%-8s ", lp->ctid ? lp->ctid : ""); break;
   case VPID: xSnprintf(buffer, n, "%*d ", Service_pidDigits, lp->vpid); break;
   #endif
   #ifdef HAVE_VSERVER
   ...
   #ifdef HAVE_DELAYACCT
   case PERCENT_CPU_DELAY: Service_printPercentage(lp->cpu_delay_percent, buffer, n, &attr); break;
   case PERCENT_IO_DELAY: Service_printPercentage(lp->blkio_delay_percent, buffer, n, &attr); break;
   case PERCENT_SWAP_DELAY: Service_printPercentage(lp->swapin_delay_percent, buffer, n, &attr); break;
   #endif
   case CTXT:
   ...
   case AUTOGROUP_NICE:
      ...
      break;
   default:
      Service_writeField(this, str, field);
      return;
   }
...
static int LinuxService_compareByKey(const Service* v1, const Service* v2, ServiceField key) {
   const LinuxService* p1 = (const LinuxService*)v1;
   const LinuxService* p2 = (const LinuxService*)v2;
   ...
   #endif
   case IO_PRIORITY:
      return SPACESHIP_NUMBER(LinuxService_effectiveIOPriority(p1), LinuxService_effectiveIOPriority(p2));
   case CTXT:
   ...
   default:
      return Service_compareByKey_Base(v1, v2, key);
   }
}

const ServiceClass LinuxService_class = {
   .super = {
   ...
   .writeField = LinuxService_writeField,
   .compareByKey = LinuxService_compareByKey
};
